...
class SoftDQN(rl_agent.AbstractAgent):
  ...
  def _get_action_probs(self, info_state, legal_actions, is_evaluation=False):
    ...
    info_state = np.array(info_state)
    q_values = self.hk_network_apply(self.params_q_network, info_state)
    legal_action_mask = np.full(self._num_actions, -np.inf)
    legal_action_mask[legal_actions] = 0
    masked_q_values = q_values + legal_action_mask
    probs = jax.nn.softmax(masked_q_values / self._alpha)
    probs = np.array(probs)
    action = int(np.random.choice(self._num_actions, p=probs))
    return action, probs
  ...
  def add_transition(self, prev_time_step, prev_action, time_step):
    ...
    legal_actions = (time_step.observations["legal_actions"][self.player_id])
    legal_actions_mask = np.full(self._num_actions, -np.inf)
  ...
  def _loss(self, param, param_target, info_states, actions, rewards,
    ...
    next_q_values = self.hk_network.apply(param_target, next_info_states)
    next_probs = jax.nn.softmax(next_q_values / self._alpha)
    discount = self._discount_factor * (1 - are_final_steps)
    target = rewards + discount * jax.vmap(jnp.dot)(next_probs, next_q_values)
  ...
  def _full_checkpoint_name(self, checkpoint_dir):
    checkpoint_filename = "_".join(["pid" + str(self.player_id)])
  ...
  def save(self, checkpoint_dir):
    ...
    """
    path = self._full_checkpoint_name(checkpoint_dir)
  ...
  def restore(self, checkpoint_dir):
    ...
    with open(path, "rb") as f:
        (self.params_q_network) = flax.serialization.from_bytes(
            (self.params_q_network), f.read()
        )
