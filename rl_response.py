...
flags.DEFINE_integer(
    ...
    "Total number of timesteps the best responder will be trained for .")
flags.DEFINE_integer(
    "save_every", int(1e4),
...
flags.DEFINE_string("exploitee", "ppo", "Exploitee (random | first | ppo)")
...
def main(_):
  print(f"game: {FLAGS.game}, exploitee: {FLAGS.exploitee}, learner: {FLAGS.learner} num_timesteps: {FLAGS.num_timesteps}")
  FLAGS.checkpoint_dir = f"tmp/{FLAGS.game}/dqn_br"
  ...
  print("INFORMATION TENSOR SHAPE: ", info_state_size)
  ...
  if FLAGS.exploitee == "first":
    ...
    print("PPO using device: %s", str(device))
    exploitee_agent = PPO(
    ...
    agent_ckpt = f"/admin/home-willb/ent-reg-marl/tmp/{FLAGS.game}/actor.pth"
    exploitee_agent.load(agent_ckpt)
    exploitee_agents = [exploitee_agent for _ in range(num_players)]
  else:
  ...
  with tf.Session() as sess:
    ...
    for ep in range(FLAGS.num_train_episodes):
      ...
      for agents in [agents_round1, agents_round2]:
        ...
        while not time_step.last():
          player_id = time_step.observations["current_player"]
          if type(agents[player_id]) == dqn.DQN:
            eval = False
          elif type(agents[player_id]) == PPO:
            eval = True
          else:
            raise RuntimeError("Unknown agent type")
          if env.is_turn_based:
            agent_output = agents[player_id].step(time_step, eval)
            action_list = [agent_output.action]
          else:
        ...
        for agent in agents:
          if type(agent) == dqn.DQN:
            agent.step(time_step)
            if agent._step_counter >= FLAGS.num_timesteps:
              agent.save(FLAGS.checkpoint_dir)
              print("Saved model to %s" % FLAGS.checkpoint_dir)
              sys.exit()
